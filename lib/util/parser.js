...
import { makeTree, createElement, createAttribute } from '../tree';
 ...
 */
const TextNode = nodeValue => assign(createElement('#text'), { nodeValue });
...
Possibly invalid markup. Saw ${match[2]}, expected ${nodeName}...
  ...
  if (root.childNodes.length && root.childNodes[0].nodeName === 'html') {
    ...
    if (!HTML.childNodes[0] || HTML.childNodes[0].nodeName !== 'head') {
      const headInstance = createElement('head');
    ...
    if (!HTML.childNodes[1] || HTML.childNodes[1].nodeName !== 'body') {
      const bodyInstance = createElement('body');
