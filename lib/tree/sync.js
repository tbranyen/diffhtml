...
export function syncTree(oldTree, newTree, patches) {
  if (!oldTree) {
    throw new Error('Missing existing tree to sync');
  }

  const oldIsCtor = oldTree && typeof oldTree.rawNodeName === 'function';
  const newIsCtor = newTree && typeof newTree.rawNodeName === 'function';

  if (oldIsCtor || newIsCtor) {
    if (oldTree && newTree && oldTree.nodeName === newTree.nodeName) {
      const node = makeNode(oldTree);
    ...
    if (newIsCtor) {
      ...
      NodeCache.set(newTree, NodeCache.get(oldTree));
    }
  }
  ...
  if (oldChildNodesLength >= childNodesLength) {
    for (let i = 0; i < childNodesLength; i++) {
      if (typeof childNodes[i].nodeName === 'function') {
        childNodes[i] = runCtor(childNodes[i], oldChildNodes[i]);
      }
      ...
      if (oldChildNodes[i].nodeName !== childNodes[i].nodeName) {
        ...
        syncTree(oldChildNodes[i], childNodes[i], patches);
      }
    }
  }
