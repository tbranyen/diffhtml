...
export default function syncTree(oldTree, newTree) {
  ...
  if (newChildNodes.length > oldChildNodes.length) {
    ...
    if (!patchset.INSERT_BEFORE) { patchset.INSERT_BEFORE = []; }
  ...
  for (let i = 0; i < oldChildNodes.length; i++) {
    ...
    if (!newChildNode) {
      ...
      if (!patchset.REMOVE_CHILD) { patchset.REMOVE_CHILD = []; }
    ...
    if (hasNoKeys && oldChildNode.nodeName !== newChildNode.nodeName) {
      if (!patchset.REPLACE_CHILD) { patchset.REPLACE_CHILD = []; }
  ...
  if (oldTree.nodeName === '#text' && newTree.nodeName === '#text') {
    if (oldTree.nodeValue !== newTree.nodeValue) {
      if (!patchset.NODE_VALUE) { patchset.NODE_VALUE = []; }
      oldTree.nodeValue = newTree.nodeValue;
      patchset.NODE_VALUE.push([oldTree, decodeEntities(oldTree.nodeValue)]);
  ...
  if (newTree.nodeType === 1) {
    ...
    if (setAttributes.length) {
      if (!patchset.SET_ATTRIBUTE) { patchset.SET_ATTRIBUTE = []; }
      patchset.SET_ATTRIBUTE.push([oldTree, setAttributes]);
    }

    if (removeAttributes.length) {
      if (!patchset.REMOVE_ATTRIBUTE) { patchset.REMOVE_ATTRIBUTE = []; }
