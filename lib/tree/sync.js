...
export default function sync(oldTree, newTree) {
  ...
  // here, because we want access to it in each recursive iteration.
  ...
  // Identical Virtual Tree Elements mean identical DOM Node's so we can safely
  // ignore syncing any further.
  ...
  if (newChildNodes.length >= oldChildNodes.length) {
    ...
    // Handle key attribute special to position offset.
      ...
      for (let i = 0; i < newChildNodes.length; i++) {
        ...
        if (key && !oldKeys.has(key) && newKeys.has(key)) {
