...
const blockTextElements = ['script', 'noscript', 'style', 'pre', 'template'];
...
export default function process(element, patches) {
  ...
  var attached = function(descriptor, fragment, parentNode) {
  ...
  if (activePromises.length) { return Promise.all(activePromises); }
}
