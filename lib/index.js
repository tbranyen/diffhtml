...
/**
 * A helper that creates Virtual Tree attributes. This can be used in place of
 * HTML parsing and is what the Babel transform will compile down to. This
 * pulls a shared pool object.
 *
 * @example
 *
 *    import { createAttribute, createElement } from 'diffhtml'
 *
 *    // Create an attribute with a pooled object.
 *    const attr = createAttribute('class', 'test');
 *    const vTree = createElement('div', [attr], 'Hello world');
 *
 *
 * @param {String} name - The name for the attribute
 * @param {String|Object} node - The value for the attribute/prop
...
export function element(element, newElement, options={}) {
  return Transaction.create(element, newElement, options).start();
}
