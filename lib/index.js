...
export function enableProllyfill() {
  ...
  Object.getOwnPropertyNames(window).filter(key => {
    return key.indexOf('HTML') === 0 && typeof window[key] === 'object';
  }).forEach(key => {
   ...
   */
  let activateComponents = function activateComponents() {
    ...
    TreeCache.forEach(function iterateTreeCache(elementMeta, element) {
