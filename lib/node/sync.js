...
export default function sync(oldTree, newTree, patches) {
  ...
  if (oldChildNodesLength > childNodesLength) {
    ...
    let newHasKeys = childNodes.some(childNode => !childNode.is);
    ...
    // key was specified.
      ...
      toRemove = shallowClone.reduce((toRemove, oldChildNode, index) => {
        let newChildNode = childNodes[index + offset];
        let oldIndex = oldChildNodes.indexOf(oldChildNode);
