...
export default function patch(element, newHTML, options) {
  ...
  if (options.enableWorker && hasWorker && elementMeta.__has_rendered__) {
    // Attach all properties here to transport.
    let transferObject = {};

    if (elementMeta.oldTree) { transferObject.oldTree = elementMeta.oldTree; }
