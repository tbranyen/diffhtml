  ...
  return function(ev) {
    ...
    if (nodes.additions.length) {
      nodes.additions.map(function(descriptor) {
        pools.elementObject._uuid[descriptor.element] = true;
        return descriptor;
      }).map(makeElement);
    }
    ...
    // cycle.
    processPatches(element, { data: ev.data.patches }).then(function() {
      // Remove unused elements.
      if (nodes.removals.length) {
        nodes.removals.forEach(function(descriptor) {
          delete pools.elementObject._uuid[descriptor.element];
          delete makeNode.nodes[descriptor.element];
        });
      }
...
export function patchNode(element, newHTML, options) {
  ...
  if (options.enableWorker && hasWorker) {
    ...
    // have completed.
    let processPromise = processPatches(element, { data });
