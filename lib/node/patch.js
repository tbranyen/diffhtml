  ...
  for (let i = 0; i < patches.length; i++) {
    ...
    for (let i = 0; i < APPEND_CHILD.length; i++) {
      ...
      for (let i = 0; i < childNodes.length; i++) {
        const vTree = makeNode(childNodes[i]);
        protectElement(vTree);
        fragment.appendChild(vTree);
      }
    ...
    for (let i = 0; i < REMOVE_CHILD.length; i++) {
      ...
      domNode.removeChild(makeNode(childNode));
      unprotectElement(childNode);
    }
    ...
    for (let i = 0; i < REPLACE_CHILD.length; i++) {
      ...
      domNode.replaceChild(makeNode(newChildNode), makeNode(oldChildNode));
      unprotectElement(oldChildNode);
      protectElement(newChildNode);
    }
