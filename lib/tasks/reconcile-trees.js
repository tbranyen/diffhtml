...
import { createElement, makeTree } from '../tree';
 ...
 */
const getNewTreeFromMarkup = (markup, options, callback) => {
  // This is HTML Markup, so we need to parse it.
  if (typeof markup === 'string') {
    // If we are dealing with innerHTML, use all the Nodes. If we're dealing
    // with outerHTML, we can only support diffing against a single element,
    // so pick the first one.
    return callback(parse(markup, null, options).childNodes);
  }
...
export default function reconcileTrees(transaction) {
  ...
  if (!sameInnerHTML || !sameOuterHTML || !sameTextContent) {
    ...
    state.oldTree = makeTree(domNode);
  ...
  transaction.newTree = getNewTreeFromMarkup(markup, options, newTree => {
    if (inner || Array.isArray(newTree)) {
      return createElement('#document-fragment', null, newTree);
    }
